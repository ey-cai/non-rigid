...
class ProcClothFlowDataset(data.Dataset):
    def __init__(self, root, dataset_cfg, type):
        ...
        size = (
            self.dataset_cfg.train_size
            if "train" in self.type
            else self.dataset_cfg.val_size
        )
        if size is not None:
            self.size = size
        else:
    ...
    def __getitem__(self, index):
        ...
        if self.sample_size_action > 0 and action_pc.shape[0] > self.sample_size_action:
            action_pc, action_pc_indices = downsample_pcd(
                action_pc.unsqueeze(0),
        ...
        anchor_pc, anchor_pc_indices = downsample_pcd(
            anchor_pc.unsqueeze(0),
        ...
        if self.scene:
            ...
            T_goal2world = T1.inverse().compose(Translate(center.unsqueeze(0)))
            ...
            if self.dataset_cfg.action_context_center_type == "center":
                ...
                raise ValueError(
                    f"Unknown action context center type: {self.dataset_cfg.action_context_center_type}"
                )

            points_action = action_pc - action_center
            points_action = T0.transform_points(points_action)
            T_action2world = T0.inverse().compose(Translate(action_center.unsqueeze(0)))
            # Get the flow
...
class DeformablePlacementDataset(data.Dataset):
    def __init__(self, root, dataset_cfg, split):
        ...
        # First index is the minimum value of the file name in the dataset dir.
        self.first_ix = min(
            [int(f.split("_")[-1].split(".")[0]) for f in os.listdir(self.dataset_dir)]
        )
        print(f"First index for split {split}: {self.first_ix}")
    ...
    def __getitem__(self, index):
        ...
        try:
            demo = np.load(
                self.dataset_dir / f"demo_{self.first_ix + file_index}.npz",
                allow_pickle=True,
            )
        # FileNotFound error is raised when the index is out of range
        except FileNotFoundError:
            # print(f"HACK we're just gonna try again with index {index + 1}")
        ...
        # object-centric dataset
            ...
            item[
                "pc_action"
            ] = action_pc  # Action points in starting position for context
...
class ProcClothFlowDataModule(L.LightningDataModule):
    def __init__(
        self, batch_size, val_batch_size, num_workers, dataset_cfg
    ):  # type, scene):
        super().__init__()
        # self.root = root
    ...
    def setup(self, stage: str = "fit"):
        ...
        if self.stage != "fit":
            print(
                "-------Turning off rotation augmentation for validation/inference.-------"
            )
        ...
        if self.dataset_cfg.world_frame:
            print(
                "-------Turning off mean-centering for world frame predictions.-------"
            )
