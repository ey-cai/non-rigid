...
from non_rigid.models.dit.models import (
    DiT_PointCloud_Cross_CFG,
)
from non_rigid.utils.logging_utils import viz_predicted_vs_gt
from non_rigid.utils.pointcloud_utils import expand_pcd


def DiT_PointCloud_Cross_CFG_xS(use_rotary, **kwargs):
    ...
    return DiT_PointCloud_Cross_CFG(depth=5, hidden_size=hidden_size, num_heads=4, **kwargs)

def DiT_PointCloud_CFG_xS(use_rotary, **kwargs):
    # hidden size divisible by 3 for rotary embedding, and divisible by num_heads for multi-head attention
    '''
    hidden_size = 132 if use_rotary else 128
    return DiT_PointCloud(depth=5, hidden_size=hidden_size, num_heads=4, **kwargs)
    '''
    raise NotImplementedError("DiT_PointCloud_CFG_xS is not implemented yet.")

# TODO: clean up all unused functions
DiT_models = {
    "DiT_PointCloud_Cross_CFG_xS": DiT_PointCloud_Cross_CFG_xS,
    "DiT_PointCloud_CFG_xS": DiT_PointCloud_CFG_xS,
}


def get_model(model_cfg):
    #rotary = "Rel3D_" if model_cfg.rotary else ""
    cross = "Cross_" if model_cfg.name == "df_cross_cfg" else ""
    # model_name = f"{rotary}DiT_pcu_{cross}{model_cfg.size}"
    model_name = f"DiT_PointCloud_{cross}CFG_{model_cfg.size}"
    return DiT_models[model_name]


class DiffusionTransformerCFGNetwork(nn.Module):
    """
    Network containing the specified Diffusion Transformer architecture with Classifier-free Guidance.
    """
...
class DenseDisplacementDiffusionCFGModule(L.LightningModule):
    """
    Generalized Dense Displacement Diffusion (DDD) module with Classifier-free Guidance that handles model 
    training, inference, evaluation, and visualization. This module is inherited and overriden by scene-level and 
...
class CrossDisplacementCFGModule(DenseDisplacementDiffusionCFGModule):
    """
    Object-centric DDD-CFG module. Applies cross attention between action and anchor objects.
    """
    def __init__(self, network, cfg) -> None:
    ...
    def get_model_kwargs(self, batch, num_samples=None):
        ...
        model_kwargs = dict(x0=pc_action, y=pc_anchor, cond=True)
        return model_kwargs
