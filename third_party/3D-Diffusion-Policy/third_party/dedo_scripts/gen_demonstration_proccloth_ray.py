...
def parse_args():
    ...
    parser.add_argument('--save-zarr', action='store_true', help='save zarr file')
...
@ray.remote(num_gpus=1)  # TBH not sure if we need gpus for this.
def run_episode(args, seed):
    ...
    if np.sum(success_sub_list) == num_holes:
        ...
        for i in range(len(tax3d_demo_list)):
            ...
            np.savez(
                os.path.join(tax3d_save_dir, f'demo_{seed}.npz'),
                **tax3d_demo
            )
            vid_frames[0].save(
                os.path.join(rollout_vids_save_dir, f'demo_{seed}.gif'),
                save_all=True,
        ...
        if args.save_zarr:
            return {
                "total_count_sub_list": total_count_sub_list,
                "success_sub_list": success_sub_list,
                "reward_sum_list": reward_sum_list,
                "action_pcd_arrays_sub_list": action_pcd_arrays_sub_list,
                "anchor_pcd_arrays_sub_list": anchor_pcd_arrays_sub_list,
                "state_arrays_sub_list": state_arrays_sub_list,
                "action_arrays_sub_list": action_arrays_sub_list,
            }
        else:
            return {}
    else:
        print("Failed on at least one hole...")
        return None
...
if __name__ == '__main__':
    ...
    # Write a for loop that runs the episodes in parallel using ray
    ray.init()
    futures = [run_episode.remote(args, seed) for seed in range(num_episodes)]
    with tqdm(total=num_episodes) as pbar:
        for future in ray.get(futures):
            if future is not None:
                if args.save_zarr:
                    success_list.extend(future["success_sub_list"])
                    reward_list.extend(future["reward_sum_list"])
                    action_pcd_arrays.extend(future["action_pcd_arrays_sub_list"])
                    anchor_pcd_arrays.extend(future["anchor_pcd_arrays_sub_list"])
                    state_arrays.extend(future["state_arrays_sub_list"])
                    action_arrays.extend(future["action_arrays_sub_list"])
                    episode_ends_arrays.extend(np.cumsum(future["total_count_sub_list"]) + total_count)
                    total_count += np.sum(future["total_count_sub_list"])
            else:
                print(f"Failed on at least one on {future}, retrying...")
            pbar.update(1)
    if not args.save_zarr:
        print("not saving zarr data, but generation completed")
        ray.shutdown()
        exit(0)
